...
-- "Forgot password" never resets anything itself — it just queues a request
-- that IT/admin sees and actions manually (matches how edit_requests already
-- works: nothing self-service touches real data without a human approving).

CREATE TYPE password_reset_status AS ENUM ('PENDING', 'RESOLVED');

CREATE TABLE password_reset_requests (
  id             uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  center_id      uuid REFERENCES centers(id) ON DELETE CASCADE, -- filled in by the backend once it resolves the identifier below; NULL if it can't find a match
  role           text NOT NULL CHECK (role IN ('admin', 'teacher', 'student')),
  identifier     text NOT NULL,             -- whatever they typed: email or School ID Number
...
CREATE INDEX idx_pwreset_center ON password_reset_requests(center_id);
CREATE INDEX idx_pwreset_status ON password_reset_requests(status);
